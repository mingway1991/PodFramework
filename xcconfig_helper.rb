    ...
    module XCConfig
      ...
      module XCConfigHelper
        ...
        def self.add_framework_build_settings(framework_path, xcconfig, sandbox_root, include_other_ldflags = true)
          ...
          tmp_path = framework_path.dirname.relative_path_from(sandbox_root).to_s
          if tmp_path.include?"LibRepo"
                location = tmp_path.index("LibRepo")
                dirname = "${HOME}/Documents/"+tmp_path[location,tmp_path.length-location]
              else
                dirname = '${PODS_ROOT}/' + tmp_path
              end
          build_settings = {
